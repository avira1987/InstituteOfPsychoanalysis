...
/**
 * @param {object} instanceDetail — status/dashboard payload
 * @param {string} triggerEvent
 * @param {object} basePayload
 * @returns {{ payload: object, error: string | null }}
 */
export function mergeEducationalLeaveTriggerPayload(instanceDetail, triggerEvent, basePayload = {}) {
  if (instanceDetail?.process_code !== 'educational_leave') {
  ...
  if (triggerEvent === 'committee_rejected') {
    ...
    if (!reason) {
      return { payload, error: 'علت رد را در فرم مرحله وارد و ثبت کنید، سپس دکمهٔ رد را بزنید.' }
    }
