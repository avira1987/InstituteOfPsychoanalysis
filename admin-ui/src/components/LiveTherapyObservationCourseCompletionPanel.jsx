...
/**
 * داشبورد مدرس — خاتمه درس مشاهده زنده درمان (گزارش PDF و نمره نهایی) — فرایند SOP ۶۵.
 */
export default function LiveTherapyObservationCourseCompletionPanel({
  detail = null,
  ...
  const courseCode = therapyCtx.courseCode
  ...
  const prefilledById = useMemo(() => {
    const map = {}
    ;(therapyCtx.studentsGrades || []).forEach((row) => {
      if (row?.student_id) map[String(row.student_id)] = row
    })
    return map
  }, [therapyCtx.studentsGrades])

  const ctxDefaults = useMemo(() => ({
    participationScore: therapyCtx.participationScore,
    attendanceScore: therapyCtx.attendanceScore,
  }), [therapyCtx.participationScore, therapyCtx.attendanceScore])
  ...
  useEffect(() => {
    ...
    if (courseCode) {
      panelApi.instructorCourseRoster(courseCode, { enrichLiveTherapyReports: true })
        .then((res) => {
          const apiRows = res.data?.roster || []
          if (apiRows.length) {
            applyRows(apiRows)
          } else if (therapyCtx.studentsGrades.length) {
            applyRows(therapyCtx.studentsGrades.map((g) => ({
        ...
        .catch((e) => {
          ...
          if (therapyCtx.studentsGrades.length) {
            applyRows(therapyCtx.studentsGrades.map((g) => ({
              student_id: g.student_id,
              name_fa: g.student_name,
              role: 'student',
  ...
  const hint = STATE_HINTS[currentState]
    || 'خاتمه درس مشاهده زنده درمان — طبق راهنمای مرحله و فرم پایین اقدام کنید.'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await processExecApi.trigger(instanceId, {
        ...
        payload: {
          course_name: therapyCtx.courseName || courseCode,
          course_code: courseCode,
          session_index: therapyCtx.sessionIndex,
          students_grades,
        },
  ...
  return (
    <div
      className="card"
      data-testid="live-therapy-observation-course-completion-panel"
    ...
    >
      ...
      <div style={{ padding: compact ? '0 0.75rem 0.75rem' : '0 1rem 1rem' }}>
        ...
        <div
          data-testid="live-therapy-observation-course-summary"
          style={{
            display: 'grid',
        ...
        >
          <InfoTile label="نام درس" value={therapyCtx.courseName} tone="#0d9488" bg="#f0fdfa" />
          <InfoTile
            label="بارم"
        ...
        {currentState === 'grades_entry' && !courseCode && !studentRows.length && (
          <LiveTherapyCompletionHintBlock title="کد درس" tone="warn">
            کد درس در زمینهٔ پرونده ثبت نشده است. از فرم پایین یا پشتیبانی، فیلد course_code را تنظیم کنید.
          </LiveTherapyCompletionHintBlock>
        )}

        <LiveTherapyCompletionHintBlock tone="info">
          نمرات مشارکت و حضور از فرایند ۷۴ (`live_therapy_observation_ta_attendance_completion`) می‌آید.
        ...
        {currentState === 'grades_entry' && (
          <>
                      ...
                      <tr key={row.student_id || idx}>
                        ...
                        <td>
                          <input
                            ...
                            data-testid={`live-therapy-report-grade-${row.student_id}`}
                            style={{ width: '5rem' }}
            ...
            {studentRows.some((r) => r.borderline) && (
              <LiveTherapyCompletionHintBlock title="نمره مرزی (۶۴–۷۳)" tone="warn">
                {BORDERLINE_SMS_FA}
              </LiveTherapyCompletionHintBlock>
            )}
            ...
            {studentRows.length > 0 && (
              <button
                ...
                data-testid="live-therapy-observation-course-submit-grades"
