...
/**
 * راهنمای کمیته نظارت برای فرایند ۷۱.
 */
export default function EducationalTherapistMonitoringReviewPanel({ detail = null, user = null }) {
  ...
  if (
    ...
    || currentState !== 'monitoring_review'
  ) {
    return null
  }
  ...
  return (
    <div
      data-testid="et-monitoring-review-panel"
      style={{
    ...
    >
      <h4 style={{ fontSize: '0.9rem', fontWeight: 700, margin: '0 0 0.5rem', color: '#92400e' }}>
        بررسی صلاحیت ارتقا به درمانگر آموزشی (فرایند ۷۱)
      </h4>

      <p style={{ fontSize: '0.85rem', lineHeight: 1.7, margin: '0 0 0.75rem', color: '#334155' }}>
        پروندهٔ متقاضی را برای ارتقا به درمانگر آموزشی بررسی کنید. فرم تصمیم را تکمیل کنید، سپس
        {' '}
      ...
      {ctx.et_eligibility_summary_fa && (
        <p style={{ fontSize: '0.82rem', margin: '0 0 0.5rem', color: '#64748b' }}>
          خلاصه احراز خودکار:
          {' '}
          <strong>{ctx.et_eligibility_summary_fa}</strong>
        </p>
      )}
