...
const ALL_SECTIONS = [
  ...
  { id: 'student_path', title: 'مسیر آموزشی از ثبت‌نام تا فراغت', roles: ['student'] },
  { id: 'step1', title: 'گام ۱: ورود به سیستم', roles: null },
  { id: 'role_student', title: 'پنل دانشجو و بخش‌های اصلی', roles: ['student'] },
...
function getSectionsForRole(userRole) {
  let list
  if (!userRole) {
    list = ALL_SECTIONS.filter((s) => !s.roles || s.roles.length === 0)
  } else if (userRole === 'admin') {
    list = ALL_SECTIONS
  } else {
    list = ALL_SECTIONS.filter((s) => {
      if (!s.roles || s.roles.length === 0) return true
      return s.roles.includes(userRole)
    })
  }
  if (userRole === 'student') {
    list = list.filter((s) => s.id !== 'intro' && s.id !== 'concepts')
    const sp = ALL_SECTIONS.find((x) => x.id === 'student_path')
    if (sp && list.some((x) => x.id === 'student_path')) {
      list = [sp, ...list.filter((x) => x.id !== 'student_path')]
    }
  }
  return list
}
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {userRole === 'student' ? (
              <>
                مسیر تحصیلی و اداری شما در یک پنل: از ثبت‌نام و پیگیری پرونده تا جلسات، درخواست‌ها و مراحل نزدیک به فراغت — بدون نیاز به دانستن جزئیات فنی سامانه.
                <span style={{ display: 'block', marginTop: '0.35rem', fontSize: '0.9rem', opacity: 0.9 }}>
                  بخش‌های «ساخت و تعریف فرایند» مخصوص همکاران اداری است و در فهرست شما نمایش داده نمی‌شود.
                </span>
              </>
            ) : (
              <>
      ...
      <div className="guide-layout">
        ...
        <div className="guide-content">
          ...
          {/* ────────────── مسیر آموزشی (فقط دانشجو) ────────────── */}
          {userRole === 'student' && (
            <section id="student_path" className="guide-section">
              <div className="guide-section-header">
                <span className="guide-section-icon">🎓</span>
                <h2>مسیر آموزشی از ثبت‌نام تا فراغت</h2>
              </div>
              <div className="guide-card">
                <p>
                  این راهنما برای <strong>شما به‌عنوان دانشجو</strong> است: یعنی چه کارهایی را در سامانه انجام می‌دهید، در هر مرحله چه انتظاری داشته باشید، و چرا برخی درخواست‌ها تا وقتی به مرحلهٔ درست نرسیده‌اید باز نمی‌شوند.
                  شما <strong>فرایندی «نمی‌سازید»</strong>؛ همان‌طور که در دفتر درخواست می‌دادید اینجا درخواست ثبت می‌کنید و مراحل بعدی توسط انستیتو و طبق مقررات پیش می‌رود.
                </p>
                <div className="guide-callout info">
                  <div className="guide-callout-icon">📌</div>
                  <div>
                    <strong>اصل کار:</strong> قدم اصلی همیشه از <strong>داشبورد</strong> و کارت <strong>«مسیر فعلی»</strong> است.
                    تب «درخواست‌های تکمیلی» فقط وقتی به آن نیاز دارید که موضوعی جدا از مسیر اصلی (مثلاً مرخصی، سوپرویژن) مطرح است و در همان ترم/مرحله برای شما مجاز شده باشد.
                  </div>
                </div>

                <h3>۱) پیش از ورود به دوره: ثبت‌نام و پذیرش</h3>
                <ul>
                  <li>ثبت‌نام اولیه معمولاً از طریق فرم عمومی سایت انجام می‌شود؛ اطلاعات را دقیق و با همان شماره‌ای که در پرونده خواهد ماند وارد کنید.</li>
                  <li>پس از ارسال، بررسی مدارک و نتیجهٔ اولیه اغلب با <strong>پیامک</strong> یا تماس اطلاع‌رسانی می‌شود؛ صبر داشته باشید و پیام‌ها را نگه دارید.</li>
                  <li>در صورت دعوت به مصاحبه یا تکمیل مدارک، طبق اعلام دفتر اقدام کنید؛ وضعیت پرونده را بعداً در همین سامانه، پس از ایجاد حساب کاربری، پیگیری می‌کنید.</li>
                </ul>

                <h3>۲) حساب کاربری، ورود و پنل دانشجو</h3>
                <ul>
                  <li>ورود معمولاً با <strong>شماره موبایل</strong> و <strong>کد یکبار مصرف پیامکی</strong> است؛ شماره باید همان باشد که در پرونده ثبت شده است.</li>
                  <li>پس از ورود، به <strong>پنل دانشجو</strong> می‌رسید: داشبورد، فرایندها، جلسات، تکالیف، درخواست‌ها، پروفایل و بخش پیشرفت/امتیاز مسیر آموزشی.</li>
                  <li>اگر پروفایل دانشجویی هنوز در سامانه فعال نشده، با دفتر تماس بگیرید؛ بدون پروفایل، بسیاری از بخش‌ها خالی می‌مانند.</li>
                </ul>

                <h3>۳) ثبت‌نام دوره و جلو بردن مسیر اصلی</h3>
                <ul>
                  <li>وقتی پذیرفته شدید، اغلب یک <strong>فرایند ثبت‌نام دوره</strong> برای شما باز می‌شود؛ فرم‌های مرحله‌ای را تکمیل کنید، مدارک یا پرداخت‌های خواسته‌شده را در همان مسیر انجام دهید.</li>
                  <li><strong>تا وقتی این مسیر اصلی باز است</strong>، بسیاری از درخواست‌های دیگر در پنل عمداً <strong>غیرفعال یا قفل</strong> می‌مانند؛ ابتدا همان مسیر را از داشبورد جلو ببرید — این رفتار به‌خاطر رعایت ترتیب آموزشی و اداری است، نه نقص سامانه.</li>
                </ul>

                <h3>۴) حین تحصیل: ترم، جلسات، درمان آموزشی و پرداخت</h3>
                <ul>
                  <li>در <strong>پروفایل</strong> معمولاً ترم فعلی، تعداد جلسات هفتگی و نوع دوره دیده می‌شود؛ اگر با اعلام رسمی دفتر یا پروندهٔ شما فرق دارد، از طریق دفتر اصلاح شود.</li>
                  <li>برای <strong>جلسات درمان آموزشی آنلاین</strong>، پس از هماهنگی و پرداخت/تأیید، لینک ورود در تب مربوطه نمایش داده می‌شود؛ تا قبل از فعال شدن لینک توسط درمانگر، طبیعی است که جلسه‌ای نبینید.</li>
                  <li><strong>دسترسی سریع</strong> (مثلاً پرداخت جلسات، مرخصی، جلسه اضافی، کنسل) فقط وقتی فعال است که در آن مرحله از مسیر تحصیلی مجاز باشید؛ اگر غیرفعال است، نشانگر توضیح می‌دهد چرا.</li>
                </ul>

                <h3>۵) فرایندها به زبان ساده</h3>
                <p>
                  هر «فرایند» در سامانه یک <strong>همان مسیر اداری درخواست</strong> است: مثلاً مرخصی تحصیلی، آغاز یا تغییر درمان آموزشی، سوپرویژن، یا یک درخواست کمیته.
                  شما مرحله را جلو می‌برید، فرم می‌دهید، گاهی منتظر تأیید کمیته یا همکار می‌مانید — همان‌طور که روی کاغذ «پرونده در دست فلان بخش» بود، اینجا «وضعیت» نمایش داده می‌شود.
                </p>

                <h3>۶) تکالیف، کمیته‌ها و سوپرویژن</h3>
                <ul>
                  <li><strong>تکالیف و ارجاع‌ها</strong> در تب مربوطه فهرست می‌شوند؛ مهلت و توضیح را همان بخش ببینید.</li>
                  <li><strong>کمیته‌ها و سوپرویژن</strong> طبق آیین‌نامه انستیتو و مرحلهٔ شما باز می‌شوند؛ اگر درخواستی در پنل نمی‌بینید، ممکن است هنوز به آن مرحله نرسیده باشید یا نیاز به هماهنگی حضوری با دفتر باشد.</li>
                </ul>

                <h3>۷) نزدیک به فراغت: رساله، آزمون‌ها و طی مراحل نهایی</h3>
                <ul>
                  <li>مراحل نهایی (مثلاً رساله، دفاع، آزمون‌های مقرر، یا معرفی به مرحلهٔ بعد) طبق اعلام دفتر آموزشی و کمیته‌ها در سامانه یا خارج از آن دنبال می‌شود؛ هرجا فرایند در پنل باز شد، همان مسیر را جلو ببرید.</li>
                  <li>اگر تردید دارید «الان باید چه فرایندی را بزنم؟»، ابتدا از <strong>داشبورد و مسیر فعلی</strong> بپرسید و در صورت ابهام با دفتر تماس بگیرید؛ کد دانشجویی و شماره تماس ثبت‌شده را هنگام تماس آماده داشته باشید.</li>
                </ul>

                <div className="guide-callout success">
                  <div className="guide-callout-icon">✅</div>
                  <div>
                    <strong>یادآوری:</strong> سامانه جایگزین ارتباط انسانی با دفتر و اساتید نیست؛ ابزار شفاف‌سازی و پیگیری است.
                    برای مسائل مالی، انضباطی، یا شرایط خاص همیشه با مسیر رسمی انستیتو هماهنگ کنید.
                  </div>
                </div>
              </div>
            </section>
          ...
          <section id="intro" className="guide-section">
            ...
            <div className="guide-card">
              <h3>سامانه انستیتو روانکاوری تهران چیست؟</h3>
              <p>
                سامانه <strong>انستیتو روانکاوری تهران (Tehran Institute of Psychoanalysis)</strong> یک سیستم اتوماسیون آموزشی مبتنی بر متادیتا (Metadata) است. این سامانه به شما اجازه می‌دهد
              ...
              <h3>تفاوت سیستم سنتی و سامانه انستیتو روانکاوری تهران</h3>
          ...
          {sections.some((s) => s.id === 'role_student') && (
            <section id="role_student" className="guide-section">
              <div className="guide-section-header">
                <span className="guide-section-icon">🎓</span>
                <h2>پنل دانشجو و بخش‌های اصلی</h2>
              </div>
              <div className="guide-card">
                <p>
                  جدول زیر همان چیزی است که در پنل با آن کار می‌کنید؛ با بخش <strong>«مسیر آموزشی از ثبت‌نام تا فراغت»</strong> در بالای همین راهنما بخوانید تا بدانید در هر مرحله چه انتظاری دارید.
                </p>
                <ul>
                  <li><strong>داشبورد:</strong> آمار کلی، <strong>کارت «مسیر فعلی»</strong> (قدم بعد شما)، فرایندهای فعال و آخرین فعالیت‌ها؛ معمولاً همان‌جا باید بمانید تا کار نیمه‌تمام تمام شود.</li>
                  <li><strong>فرایندها:</strong> همهٔ درخواست‌های جاری و گذشته؛ برای ادامهٔ یک فرایند باز، همان را باز کنید. برای شروع نوع جدید، اگر دکمه در داشبورد نبود، از «درخواست‌های تکمیلی» با توجه به توضیح قفل بودن استفاده کنید.</li>
                  <li><strong>درخواست‌های تکمیلی:</strong> فهرست درخواست‌هایی که ممکن است در ترم‌های بعدی به آن‌ها نیاز شود؛ فقط اگر برای مرحلهٔ شما مجاز باشد دکمه فعال است.</li>
                  <li><strong>جلسات آنلاین درمان:</strong> تاریخ، وضعیت پرداخت/جلسه و لینک ورود وقتی توسط درمانگر فعال شده باشد.</li>
                  <li><strong>تکالیف:</strong> عنوان، مهلت و ارسال پاسخ متنی در همان تب.</li>
                  <li><strong>پروفایل:</strong> اطلاعات شخصی و تحصیلی (ترم، جلسات هفتگی، درمان آموزشی، کارآموزی) و در صورت وجود، کارت راهنمای «فرایند فعلی و تکلیف شما».</li>
                  <li><strong>پیشرفت، رتبه و مدال‌ها:</strong> امتیاز تجربه (XP) و انگیزهٔ پیگیری مسیر؛ جایگزین تصمیم آموزشی یا نمره نیست.</li>
                </ul>
                <div className="guide-callout info">
                  <div className="guide-callout-icon">💡</div>
                  <div>
                    <strong>نکته:</strong> اگر دکمه‌ای خاموش است، روی آن بمانید (یا عنوان را بخوانید) تا ببینید چرا — اغلب به‌خاطر باز بودن ثبت‌نام دوره، داشتن همان فرایند به‌صورت فعال، یا نرسیدن به ترم/شرط مقرر است.
                  </div>
                </div>
              </div>
            </section>
            ...
            <section id="role_staff" className="guide-section">
              ...
              <div className="guide-card">
                ...
                <div className="guide-callout info">
                  <div className="guide-callout-icon">💡</div>
                  <div>
                    <strong>نکته:</strong> از منو به «ردیابی دانشجو» و «گزارش حسابرسی» دسترسی دارید.
                  </div>
                </div>
              </div>
            </section>
          ...
          <section id="step2" className="guide-section">
            ...
            <div className="guide-card">
              ...
              <p style={{ marginBottom: '1rem', color: 'var(--text-secondary)' }}>
                فهرست و کاتالوگ تعاریف فرایند در همین رابط وب نمایش داده نمی‌شود؛ ایجاد رکورد فرایند از طریق API ادمین،
                همگام‌سازی متادیتا، یا ابزارهای فنی انجام می‌شود. پس از ایجاد، ویرایش مراحل از صفحهٔ همان فرایند.
              </p>
              <div className="guide-steps">
                <div className="guide-step">
                  <div className="guide-step-number">1</div>
                  <div className="guide-step-content">
                    <h4>شناسهٔ فرایند را داشته باشید و صفحهٔ ویرایش را باز کنید</h4>
                    <p>
                      آدرس{' '}
                      <code dir="ltr" style={{ fontSize: '0.85rem' }}>/panel/processes/{'{شناسه}'}</code>
                      {' '}را در مرورگر بزنید (شناسه همان <code dir="ltr">id</code> رکورد در دیتابیس است).
                    </p>
                  </div>
                </div>
                <div className="guide-step">
                ...
                <div className="guide-step">
                  <div className="guide-step-number">3</div>
                  <div className="guide-step-content">
                    <h4>ذخیره و ادامهٔ تعریف وضعیت‌ها و انتقال‌ها</h4>
                    <p>پس از ثبت فیلدهای پایه، در همان صفحهٔ ویرایش تب‌های وضعیت‌ها و انتقال‌ها را تکمیل کنید.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section id="faq" className="guide-section">
            ...
            <div className="guide-card">
              {userRole === 'student' ? (
                <>
                  <FaqItem
                    question="چرا بعضی دکمه‌های درخواست در پنل من غیرفعال است؟"
                    answer="معمولاً به‌خاطر یکی از این موارد است: هنوز فرایند اصلی (مثلاً ثبت‌نام دوره) باز مانده؛ همان نوع درخواست را قبلاً شروع کرده‌اید؛ یا طبق مقررات انستیتو هنوز به ترم/شرط لازم نرسیده‌اید. روی دکمه بمانید یا متن راهنما را بخوانید؛ در شک، با دفتر تماس بگیرید."
                  />
                  <FaqItem
                    question="فرق داشبورد با تب «فرایندها» چیست؟"
                    answer="داشبورد جمع‌بندی و کارت «مسیر فعلی» را نشان می‌دهد — همان‌جا باید بدانید قدم بعد شما چیست. تب «فرایندها» فهرست کامل درخواست‌های جاری و گذشته و جزئیات مراحل هر کدام است."
                  />
                  <FaqItem
                    question="آیا باید برای هر کار کوچک یک «فرایند» جدید بسازم؟"
                    answer="خیر. شما فرایند نمی‌سازید؛ فقط درخواست‌های از پیش تعریف‌شده توسط انستیتو را شروع یا ادامه می‌دهید. ساخت و ویرایش فرایندها کار همکاران اداری و فنی است."
                  />
                  <FaqItem
                    question="ورود به سامانه چگونه است؟"
                    answer="ورود معمولی با شماره موبایل ثبت‌شده در پرونده و کد یکبار مصرف پیامکی است. اگر نام کاربری و رمز هم دارید، از همان مسیر اعلام‌شده توسط انستیتو استفاده کنید."
                  />
                  <FaqItem
                    question="جلسه آنلاین در پنل نیست؛ طبیعی است؟"
                    answer="تا وقتی پرداخت/تأییدهای لازم انجام نشده و درمانگر لینک را فعال نکرده، ممکن است جلسه‌ای دیده نشود. اگر مدت‌هاست وضعیت تغییر نکرده، با دفتر یا هماهنگ‌کننده جلسات تماس بگیرید."
                  />
                  <FaqItem
                    question="امتیاز XP و مدال‌ها روی نمره یا ترم اثر می‌گذارد؟"
                    answer="خیر؛ بخش انگیزشی و نمایشی است و جایگزین تصمیم آموزشی، کمیته یا مقررات رسمی نیست."
                  />
                </>
              ) : (
                <>
