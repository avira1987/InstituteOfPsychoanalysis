#!/usr/bin/env python3
"""
راهنمای PDF اپراتور — فرایند ۱۹: آغاز سوپرویژن فردی دوم یا سوم یا چهارم یا پنجم
(بدون جزئیات فنی — برای آزمایش UI و صحت پیاده‌سازی)

اجرا از ریشهٔ ریپو:
  python scripts/generate_supervision_block_start_19_guide_pdf.py

خروجی:
  docs/راهنمای_تست_فرایند_۱۹_آغاز_سوپرویژن_فردی_بلوک_بعدی.pdf
"""
...
OUT_PDF = ROOT / "docs" / "راهنمای_تست_فرایند_۱۹_آغاز_سوپرویژن_فردی_بلوک_بعدی.pdf"
...
PROCESS_TITLE = "آغاز سوپرویژن فردی دوم یا سوم یا چهارم یا پنجم"
PROCESS_UI_NAME = "آغاز سوپرویژن فردی دوم… (فرایند ۱۹)"
...
class GuidePDF(FPDF):
    ...
    def footer(self) -> None:
        ...
        self.cell(
            ...
            5,
            _fa(
                f"راهنمای تست فرایند ۱۹ — آغاز بلوک سوپرویژن — {self._footer_ts} — ص {self.page_no()}"
            ),
            align="C",
        )
...
STAGES = [
    {
        ...
        "student_sees": (
            ...
            f"«{PROCESS_UI_NAME}»؛ "
            "شمارندهٔ «حضور در بلوک فعلی» (مثلاً ۵۰ از ۵۰) و "
            "«تا جلسه ۵۰»؛ نوار چهار مرحله‌ای مسیر پرداخت در بالای بلوک راهنما."
        ),
        "student_does": (
            "راهنما را بخواند؛ دکمهٔ «ادامه و ثبت مرحله» را بزند "
            "تا سامانه بررسی کند آیا واقعاً به جلسهٔ پنجاهم رسیده است."
        ),
        "operator_checks": [
            "عنوان فرایند ۱۹ و بلوک بنفش راهنما دیده می‌شود.",
            "اگر هنوز به جلسه ۵۰ نرسیده، پیام یا هدایت منطقی (نه خطای نامفهوم) باشد.",
            "اگر به جلسه ۵۰ رسیده، مرحلهٔ بعد (سوابق و وقت‌ها) باز شود.",
            "متن «بلوک هدف» (دوم، سوم، چهارم یا پنجم) در صورت نمایش، منطقی باشد.",
        ],
    },
    {
        "name": "نمایش سوابق و وقت‌های آزاد",
        "student_sees": (
            "پیام نارنجی «الزام انتقال بلوک» با متن: "
            "«اول زمان و سوپرویژن بعدی را انتخاب کنید…»؛ "
            "جدول «سوابق بلوک‌های ۵۰ ساعته»؛ "
            "جدول «وقت‌های آزاد سوپروایزرها»؛ "
            "فرم انتخاب سوپروایزر، روز و ساعت."
        ),
        "student_does": (
            "سوابق گذشته را مرور کند؛ "
            "در فرم پایین صفحه یک سوپروایزر و یک روز و ساعت انتخاب کند؛ "
            "فرم را ثبت کند؛ سپس «ادامه و ثبت مرحله» را بزند."
        ),
        "operator_checks": [
            "پیام الزام دقیقاً معنای «قبل از پرداخت جلسه ۵۰، بلوک بعد را مشخص کن» را می‌رساند.",
            "جدول سوابق (در صورت داده) سوپروایزر و ساعات گذشته را نشان می‌دهد.",
            "در راهنما «حداکثر ۱ جلسه در هفته» ذکر شده باشد.",
    ...
    },
    {
        ...
        "student_sees": (
            "پیش‌نمایش «انتخاب شما» (سوپروایزر، روز، ساعت)؛ "
            "«تاریخ آغاز (قانون ۲۴ ساعت)»؛ "
            "بخش «پرداخت آنلاین» با توضیح «پرداخت جلسه اول دوره سوپرویژن جدید»."
        ),
        "student_does": (
        ...
        "operator_checks": [
            "توضیح پرداخت مربوط به «جلسه اول بلوک بعد» باشد، نه جلسه ۵۰.",
            "تاریخ شروع منطقی است (اگر کمتر از ۲۴ ساعت مانده، هفته بعد).",
            "پس از پرداخت موفق، «قفل پرداخت جلسه ۵۰ — باز» نمایش داده شود.",
        ],
    },
    {
        ...
        "student_sees": (
            ...
            "بخش پرداخت با توضیح «پرداخت جلسه ۵۰ام دوره سوپرویژن فعلی»."
        ),
        "student_does": (
        ...
        "operator_checks": [
            "قبل از پرداخت اول، امکان پرداخت جلسه ۵۰ نباید باز باشد.",
            "پس از پرداخت دوم، وضعیت «تکمیل» دیده شود.",
            "نام سوپروایزر جدید و تاریخ آغاز در پیام پایانی باشد.",
        ],
    },
    {
        ...
        "student_sees": (
            ...
            "فرایند بسته شده؛ "
            "در «تاریخچه پیامک» همان کارت، متن اطلاع‌رسانی به دانشجو و سوپروایزر جدید."
        ),
        "student_does": "نیازی به اقدام نیست؛ وضعیت نهایی را مرور کند.",
        "operator_checks": [
            "فرایند در لیست «تکمیل‌شده» قرار گرفته باشد.",
            "پیامک (در آزمایش: تاریخچه در پورتال) ثبت شده باشد.",
...
class PdfBuilder:
    ...
    def cover(self) -> None:
        ...
        p.cell(0, 10, _fa(f"فرایند ۱۹ — {PROCESS_TITLE}"), align="C", new_x="LMARGIN", new_y="NEXT")
        p.set_font("Vazir", "", 10)
        p.cell(
            0,
            8,
        ...
        p.multi_cell(
            ...
            6,
            _fa(
                "این کتابچه به شما کمک می‌کند صفحهٔ «آغاز بلوک بعدی سوپرویژن فردی» "
        ...
        for label, blank in [
            ...
            ("بلوک هدف آزمایش (دوم / سوم / چهارم / پنجم)", "________________________"),
        ...
        self._warn_box(
            ...
            "و پروندهٔ نمونهٔ دانشجو (نزدیک یا در جلسهٔ ۵۰ سوپرویژن) آماده باشد."
        )
    ...
    def part_what_is_process(self) -> None:
        ...
        self._body(
            "وقتی دانشجو یک «بلوک ۵۰ ساعته» سوپرویژن فردی را به پایان می‌رساند "
            "(جلسهٔ پنجاهم) و می‌خواهد همان جلسه را بپردازد، سامانه یک قانون مهم دارد:"
        )
        self._warn_box(
            "دانشجو باید اول سوپروایزر و زمان «بلوک بعدی» (دوم، سوم، چهارم یا پنجم) "
            "را انتخاب کند و هزینهٔ جلسهٔ اول آن بلوک را بپردازد؛ "
            "بعد می‌تواند جلسهٔ پنجاهم بلوک فعلی را پرداخت کند."
        )
        self._body("چرا این کار لازم است؟", bold=True)
        self._bullet_list([
            "بین دو دورهٔ ۵۰ ساعته وقفهٔ طولانی پیش نیاید.",
            "دانشجو قبل از پایان یک دوره، برنامهٔ دورهٔ بعد را مشخص کند.",
            "سوابق گذشته (با کدام سوپروایزر چند ساعت) شفاف دیده شود.",
            "انتقال منظم از بلوک اول به دوم، سوم، چهارم و پنجم حفظ شود.",
        ])
    ...
    def part_ui_on_screen(self) -> None:
        ...
        self._body(
            f"وقتی فرایند را در پورتال دانشجو باز می‌کنید، "
            f"بلوک بنفش راهنما با عنوان «{PROCESS_UI_NAME}» "
            "و این بخش‌ها دیده می‌شود:"
        )
        ui_parts = [
            (
                "نوار چهار مرحله‌ای",
                "۱) انتخاب سوپروایزر و زمان — ۲) پرداخت جلسه اول بلوک بعد — "
                "۳) پرداخت جلسه ۵۰ — ۴) تکمیل و اطلاع‌رسانی."
            ),
            (
                "بلوک هدف",
                "مثلاً «سوپرویژن فردی دوم» یا «سوم» — متناسب با پروندهٔ دانشجو."
            ),
            (
                "شمارندهٔ پیشرفت",
                "«حضور در بلوک فعلی» (مثلاً ۵۰ از ۵۰) و «تا جلسه ۵۰»."
            ),
            (
                "پیام الزام (نارنجی)",
                "«دانشجوی گرامی، برای پرداخت جهت حضور در ۵۰مین جلسه… "
                "اول زمان و سوپرویژن بعدی خود را انتخاب کنید…»"
            ),
            (
                "جدول سوابق",
            ...
            (
                "بخش پرداخت آنلاین",
                "در مراحل پرداخت: دکمهٔ ورود به درگاه."
            ),
            (
    ...
    def part_flow_simple(self) -> None:
        ...
        self._numbered_list([
            "دانشجو می‌خواهد جلسهٔ پنجاهم سوپرویژن فعلی را بپردازد.",
            "فرایند «آغاز سوپرویژن فردی دوم… (فرایند ۱۹)» را در پورتال باز می‌کند.",
            "راهنمای صفحه را می‌خواند (پیشرفت بلوک، نوار چهار مرحله‌ای).",
            "«ادامه و ثبت مرحله» را می‌زند.",
            "سامانه بررسی می‌کند: آیا واقعاً به جلسه ۵۰ رسیده؟",
            "اگر نه → پیام مناسب یا هدایت به پرداخت عادی.",
            "اگر بله → سوابق گذشته + لیست وقت‌های آزاد سوپروایزرها.",
            "یک سوپروایزر و یک زمان (حداکثر ۱ جلسه در هفته) انتخاب می‌کند.",
            "تاریخ شروع با «قانون ۲۴ ساعت» محاسبه می‌شود.",
            "جلسهٔ اول بلوک بعد (دوم، سوم، …) را پرداخت می‌کند.",
            "قفل جلسه ۵۰ باز می‌شود.",
            "جلسه ۵۰ام بلوک فعلی را پرداخت می‌کند.",
    ...
    def part_before_start(self) -> None:
        ...
        self._numbered_list([
            ...
            "دانشجو حداقل یک بلوک ۵۰ ساعته را گذرانده یا در حال اتمام آن باشد.",
            "ترجیحاً تعداد حضور در بلوک فعلی به ۵۰ رسیده باشد "
            "(مسیر «هنوز به ۵۰ نرسیده» را هم جداگانه امتحان کنید).",
            "حداقل یک سوپروایزر با «وقت آزاد» در سامانه ثبت شده باشد.",
            "فرایند «آغاز سوپرویژن فردی…» برای همان دانشجو باز یا قابل شروع باشد.",
        ])
        self._body(
            "اگر حساب یا پروندهٔ آماده ندارید، از مسئول پروژه بخواهید "
            "«پروندهٔ نمونه برای فرایند ۱۹» را آماده کند — "
    ...
    def part_where_ui(self) -> None:
        ...
        places = [
            ...
            (
                ...
                f"اگر این فرایند فعال باشد، بلوک بنفش راهنما "
                f"«{PROCESS_UI_NAME}» "
                "و خلاصهٔ پیشرفت دیده می‌شود."
            ),
            (
                "تب «فرایندها»",
                "از لیست، «مدیریت تغییرات سوپرویژن فردی» یا "
                "«آغاز سوپرویژن فردی…» را باز کنید. "
                "کارت کامل با جداول سوابق، وقت‌ها و فرم مرحله اینجاست."
            ),
            (
                "تب «درخواست‌های دیگر»",
                "اگر فرایند خودکار باز نشده، دکمهٔ شروع مربوطه را ببینید "
                "(در صورت مجاز بودن برای آن دانشجو)."
            ),
        ]
    ...
    def part_operator_scenario(self) -> None:
        ...
        self._numbered_list([
            ...
            "به تب «فرایندها» بروید و فرایند سوپرویژن (فرایند ۱۹) را باز کنید.",
            "بلوک بنفش راهنما را بخوانید: «حضور در بلوک فعلی»، «تا جلسه ۵۰»، نوار چهار مرحله.",
            "اگر «بلوک هدف» (دوم/سوم/…) نمایش داده شد، با پرونده مقایسه کنید.",
    ...
    def part_rules(self) -> None:
        ...
        self._body(
            ...
            "در این مرحله انتخاب کند."
        )
        self._body("انتخاب سوپروایزر متناسب با شماره بلوک:", bold=True)
        self._bullet_list([
            "بلوک دوم (پس از اول): سوپروایزر از بین اعضای هیئت علمی پیوسته ۱ تا ۵.",
            "بلوک سوم و بالاتر: سوپروایزر از اعضای هیئت علمی ۶ به بالا.",
    ...
    def part_negative_scenarios(self) -> None:
        ...
        scenarios = [
            (
                ...
                "باید پیام مناسب ببیند و به مسیر پرداخت عادی هدایت شود."
            ),
            (
                "فرم ناقص",
            ...
            (
                "ترتیب پرداخت",
                "قبل از پرداخت اول، بررسی کنید بخش پرداخت جلسه ۵۰ "
                "باز نیست یا پیام «قفل» دارد."
            ),
        ]
    ...
    def part_checklist(self) -> None:
        ...
        checks = [
            ...
            f"بلوک راهنما «{PROCESS_UI_NAME}» دیده می‌شود",
            "نوار چهار مرحله‌ای مسیر پرداخت نمایش داده می‌شود",
            "«حضور در بلوک فعلی» و «تا جلسه ۵۰» درست نمایش داده می‌شود",
            "«بلوک هدف» (دوم/سوم/…) در صورت نمایش، منطقی است",
