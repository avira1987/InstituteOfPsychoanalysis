#!/usr/bin/env python3
"""تولید فایل Word راهنمای تست اتصال دادهٔ آماده‌سازی ترم — راست‌چین با فونت ایران‌سنس.

اجرا از ریشهٔ ریپو:
  python scripts/generate_term_offerings_data_flow_test_guide_docx.py

خروجی:
  docs/راهنمای_تست_ساده_اتصال_داده_آماده_سازی_ترم.docx
"""
...
OUT_DOCX = ROOT / "docs" / "راهنمای_تست_ساده_اتصال_داده_آماده_سازی_ترم.docx"
OUT_DOCX_ASCII = ROOT / "docs" / "term_offerings_data_flow_test_guide.docx"

SOURCES = [
    ROOT / "docs" / "راهنمای_تست_ساده_اتصال_داده_آماده_سازی_ترم.md",
]
...
def _configure_document(doc: Document) -> None:
    ...
    # Do not add w:bidiVisual to settings — Word reports the file as corrupted.
...
def markdown_to_docx(doc: Document, md_path: Path) -> None:
    ...
    while i < len(lines):
        ...
        if stripped.startswith(">"):
            _add_paragraph(doc, stripped.lstrip("> ").strip(), size=10.5)
            i += 1
            continue
...
def _embed_fonts(docx_path: Path) -> None:
    ...
    if doc_rels_path.exists():
        ...
        if not any(rel.get("Type") == font_rel_type for rel in doc_rels.findall(f"{{{REL_NS}}}Relationship")):
            doc_rids = {rel.get("Id", "") for rel in doc_rels.findall(f"{{{REL_NS}}}Relationship")}
            n = 1
            while f"rId{n}" in doc_rids:
                n += 1
            rid = f"rId{n}"
            rel = ET.Element(f"{{{REL_NS}}}Relationship")
            rel.set("Id", rid)
...
def main() -> None:
    ...
    run = cover.add_run("راهنمای تست ساده\nاتصال دادهٔ آماده‌سازی ترم")
    ...
    sub_run = sub.add_run("۱۰ سناریو — دروس، شهریه و برنامهٔ کلاسی از آماده‌سازی ترم")
    ...
    try:
        ...
        doc.save(str(out_path))
        print("توجه: فایل قبلی باز بود؛ نسخه جدید ذخیره شد.")
    # Font embedding rewrites fontTable/Content_Types in a way Word rejects on many installs.
    # IRANSans is applied in run styles; install the font locally for identical rendering.
    if __import__("os").environ.get("EMBED_FONTS") == "1":
        _embed_fonts(out_path)
    shutil.copy2(out_path, OUT_DOCX_ASCII)
    print(f"OK: {out_path}")
    print(f"OK: {OUT_DOCX_ASCII}")
