...
$cmd = "cd /opt/anistito && echo '--- Docker build ---' && docker build -t anistito-api . 2>&1 && echo '--- Docker run ---' && docker rm -f anistito-api 2>/dev/null || true && docker run -d --name anistito-api --network anistito-net -p 8000:8000 -e DATABASE_URL=postgresql+asyncpg://anistito:anistito@anistito-db:5432/anistito -e DATABASE_URL_SYNC=postgresql://anistito:anistito@anistito-db:5432/anistito -e REDIS_URL=redis://anistito-redis:6379/0 -e DEBUG=false -e SECRET_KEY=anistito-prod-secret$smsEnv anistito-api:latest sh -c 'python -m alembic upgrade head 2>/dev/null || true && python -m uvicorn app.main:app --host 0.0.0.0 --port 8000' 2>&1 && sleep 8 && echo '--- Container status ---' && docker ps -a --filter name=anistito-api && echo '--- Last logs ---' && docker logs anistito-api --tail 30 2>&1 && echo '--- Health ---' && curl -s http://127.0.0.1:8000/health 2>&1 || echo 'curl failed'"
