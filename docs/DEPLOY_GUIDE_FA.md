...
## مخزن Git و بستهٔ استقرار

فایل‌های آرشیو مثل `anistito-deploy.tgz` یا هر `*.tgz` تولیدشده برای آپلود را **در Git commit نکنید**؛ در `.gitignore` نادیده گرفته می‌شوند. مهاجرت دامنه و callback پرداخت را طبق `docs/MIGRATION_BPMS_TO_LMS.md` روی سرور تکمیل کنید.
