...
echo ''
echo '=== 3. پورت 8000 ==='
ss -tlnp 2>/dev/null | grep 8000 || netstat -tlnp 2>/dev/null | grep 8000 || echo 'پورت 8000 باز نیست'

echo ''
echo '=== 4. تست health ==='
curl -s -w '\nHTTP_CODE:%{http_code}' http://127.0.0.1:8000/health 2>&1 || echo 'curl failed'
...
Write-Host "If port 8000 not listening: container not running, run deploy again"
