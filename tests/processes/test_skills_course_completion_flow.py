...
class TestSkillsCourseCompletionFlow:
    ...
    async def test_skills_course_completion_loads_and_starts(
        ...
        """فرایند skills_course_completion لود و استارت می‌شود؛ state اول grades_entry است."""
        processes_dir = Path(__file__).resolve().parent.parent.parent / "metadata" / "processes"
        process_file = processes_dir / "skills_course_completion.json"
    ...
    async def test_skills_course_completion_flow_to_grades_locked(
        self, db_session: AsyncSession, sample_student, sample_user
    ):
