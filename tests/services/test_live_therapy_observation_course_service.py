"""Tests for live_therapy_observation_course_service."""
...
def test_course_code_from_context_prefers_course_code():
    assert course_code_from_context({"course_code": "live_therapy_1"}) == "live_therapy_1"


@pytest.mark.asyncio
class TestLiveTherapyObservationCourseService:
    async def test_collect_student_final_reports_by_course(
        ...
        inst = ProcessInstance(
            ...
            context_data={
                "course_code": "live_therapy_obs_1",
        ...
        reports = await collect_student_final_reports(db_session, "live_therapy_obs_1")
